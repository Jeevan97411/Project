...
from[dbo].[Orders] o inner join [dbo].[Shippers] s on o.ShipVia=s.ShipperID where OrderID<10300;

--20. Categories, and the total products in each category
select c.CategoryName, count(p.ProductID) as TotalProducts
from[dbo].[Categories] c join [dbo].[Products] p on c.CategoryID=p.CategoryID group by c.CategoryName order by TotalProducts desc
